---
title: "Forecasting 2020 Governor Races"
output: 
  html_document: 
    toc: yes
    toc_float:
...
target_day <- min(date, election_day)
...
We also discard other party candidates and their ballots. In some states like Alaska and NM, the third-party candidate is quite compelling...

`TODO`: question_id vs, poll_id: a poll may have multiple questions... Now I assume that each question is a small poll.
...
state_day_sigma_scaler = 0.01655
# 0.0165 from the eco
# 0.032 if we want to double it

states_poll_bias_scaler = .076#.0.15 
# result in +- 0.07585401
...
if (FLAG_RUN_MODEL) {
  ...
  # save model
  write_rds(out, sprintf('rds/out_%s.rds', target_day), compress = 'gz')
  
  write_rds(fit, sprintf('rds/fit_%s.rds', target_day), compress = 'gz')
  
  rm(fit)
  gc(full = T)
  
} else {
  out <- read_rds(sprintf('rds/out_%s.rds', target_day))
}
```
...
today_prediction <- df_prediction %>%
  filter(day == today) %>% 
...
today_prediction %>% 
  ...
  scale_x_continuous(limits = c(.24, .8), breaks = c(.25, .375, .5, .625, .75), labels = c('+50', '+25', '0', '+25', '+50'))
...
```{r fig.height=60, fig.width=20, include=FALSE, eval=FALSE}
## model diagnotics

draws <- sample(1:20000, size= 100)

df_poll_noise_sub <- df_poll_noise %>%
 # filter(.draw %in% draws) %>%
  arrange(.draw) %>% 
  rename(poll_noise = posterior)

df_states_sigma_sub <- df_sigmas %>%
  #filter(.draw %in% draws) %>%
  arrange(.draw) %>% 
  rename(sigma = .value)
...
for(poll in unique(df_poll_noise_sub$id)){
  for(state_i in unique(df_sigmas$state_index)){
   p <- df_poll_noise_sub %>% filter(id == poll) %>% pull(poll_noise)
   s <- df_states_sigma_sub %>% filter(state_index == state_i) %>% pull(sigma)
   l <- append(l, cor(p, s))
  }
}
...
posterior_dem_today <- df_prediction   %>%   
                    filter(day == today) %>%    
                    rename(dem = prediction)
```
...
```{r}
posterior_draws_today <- posterior_dem_today %>%
...
day_state_summary <- posterior_draws_today %>% 
    group_by(state) %>% 
    mean_qi(dem, rep) %>% 
    dplyr::select(-c('.width', '.point', '.interval'))


density_bin_dem_today <-  posterior_draws_today %>% 
    mutate(bin_dem = as.factor(bin_dem)) %>% 
    group_by(state, bin_dem) %>% 
    summarise(n_bin = n(), density_bin = n_bin / N_pdraws) %>% 
    mutate(bin_name = as.numeric(as.character(bin_dem)) * 100)


density_bin_rep_today <- posterior_draws_today %>% 
     mutate(bin_rep = as.factor(bin_rep)) %>% 
    group_by(state, bin_rep) %>% 
    summarise(n_bin = n(), density_bin = n_bin / N_pdraws) %>% 
    mutate(bin_name = (100 - as.numeric(as.character(bin_rep))) * 100)
...
for(s in states) {
  ...
  today_summary <- day_state_summary %>% filter(state == s)
  
  # plinko
  start_dem <-
    floor(today_summary['dem'][[1]] * 100 / bin_width) * bin_width
  start_rep <-
    floor(today_summary['rep'][[1]] * 100 / bin_width) * bin_width
  bin_n_dem <-
    ceiling(4 * today_summary['dem.var'][[1]] / bin_width ^ 2)
  bin_n_rep <-
    ceiling(4 * today_summary['rep.var'][[1]]  / bin_width ^ 2)
  max_density <-
    max(
      max(density_bin_rep_today$density_bin),
      max(density_bin_dem_today$density_bin)
    )
  ...
  plinko_steps <- draws %>%
    filter(state == s)  %>%
    mutate(
      diff_rep = bin_rep - start_rep - trailer,
      diff_dem = bin_dem - start_dem - trailer,
